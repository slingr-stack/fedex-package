...
function handleRequestWithRetry(requestFn, options, callbackData, callbacks) {
    try {
        ...
        sys.logs.info("[fedex] Handling request...: "+ JSON.stringify(error));
        methodOnInit();
        return requestFn(options, callbackData, callbacks);
    }
}
...
exports.get = function(path, httpOptions, callbackData, callbacks) {
    ...
    return httpService.get(FedEx(options), callbackData, callbacks);
};
...
exports.post = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.post(FedEx(options), callbackData, callbacks);
};
...
exports.put = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.put(FedEx(options), callbackData, callbacks);
};
...
exports.patch = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.patch(FedEx(options), callbackData, callbacks);
};
...
exports.delete = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.delete(FedEx(options), callbackData, callbacks);
};
...
exports.head = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.head(FedEx(options), callbackData, callbacks);
};
...
exports.options = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.options(FedEx(options), callbackData, callbacks);
};
...
exports.utils = {
    ...
    getConfiguration: function (property) {
        if (!property) {
            sys.logs.debug('[fedex] Get configuration');
            return JSON.stringify(config.get());
        }
        sys.logs.debug('[fedex] Get property: '+property);
...
function setApiUri(options) {
    ...
    options.url = config.get("FEDEX_API_BASE_URL") + url;
    sys.logs.debug('[fedex] Set url: ' + options.path + "->" + options.url);
    return options;
}

function setRequestHeaders(options) {
    let headers = options.headers || {};
    sys.logs.debug('[fedex] Set header apikey');
    headers = mergeJSON(headers, {"Authorization": "Bearer " + sys.storage.get('fedex - access_token',{decrypt:true})});
...
function methodOnInit() {
    let response = httpService.post({
        url: config.get("FEDEX_API_BASE_URL") + "/oauth/token",
        headers: {
    ...
    sys.logs.debug('[fedex] Response: ' + response.access_token);
    sys.storage.put('fedex - access_token', response.access_token, {encrypt: true});
}
